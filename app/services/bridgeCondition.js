...
function calculateBridgeCondition(bridge = DEFAULT_BRIDGE, params) {
  ...
  if (damages.foundations) {
    if (
      ~damages.foundations.indexOf(
        damageTypeScoring.foundationGroundDecay.match
      )
    ) {
      // find details
      _.each(damageTypeScoring.foundationGroundDecay.details, function(
        detailScore,
        match
      ) {
        ...
        if (regex.test(damages.foundationsDetail)) {
          ...
          scores.push(
            detailScore * damageTypeScoring.foundationGroundDecay.score
          )
          damageCount++
    ...
    if (~damages.foundations.indexOf(damageTypeScoring.foundationDecay.match)) {
      ...
      scores.push(
        damageTypeScoring.foundationDecay.score *
          damageTypeScoring.foundationDecay.detailsScore
      )
      damageCount++
      mechanicalDamageCount++
    }
  ...
  structuralProperties.forEach(prop => {
    ...
    _.each(paramGroup.details, function(severityOptions, damageType) {
      ...
      if (damageTypeRegEx.test(type)) {
        const isMechanical = /Mechanical|No bearings|Bearings displaced/.test(
          type
        )
  ...
  if (damages.nonStructural) {
    _.each(damageTypeScoring.nonStructural.details, function(
      detailScore,
      match
    ) {
      const regex = new RegExp(match, 'gi')
      if (regex.test(damages.nonStructural)) {
        ...
        scores.push(detailScore * damageTypeScoring.nonStructural.score)
      }
    })
  }
  ...
  if (mechanicalDamageCount > 0) {
    ...
    const y =
      0.0018 * Math.pow(mechanicalDamageCount, 3) -
      0.0305 * Math.pow(mechanicalDamageCount, 2) +
      0.0302 * mechanicalDamageCount +
      0.9862
    score *= y
  }
